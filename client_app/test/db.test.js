...
describe("Database", () => {
    ...
    describe("Parkings", () => {
        it("Get parkings shall return correct data", async () => {
            let data = [{id: 1}, {id: 2}]
            callDatabaseStub.returns({parkingspaces: data});

            let parkings = await db.getParkings();

            expect(parkings).to.be.an('array');
            expect(parkings.length).to.be.equal(2);
            expect(parkings[0].id).to.be.equal(1);
            expect(parkings[1].id).to.be.equal(2);
            expect(callDatabaseStub.calledOnce).to.be.true;
        })
    })
})
