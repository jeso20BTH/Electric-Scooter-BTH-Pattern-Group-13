...
const bank = {
  view: function () {
    const invoiceId = m.route.param('id')
    const invoice = userModel.filteredHistory[invoiceId]
    const sumary = utilitiesModel.getSumary(invoice, invoiceId, userModel.currentUser.id)
    return [
      m('h1', 'Betalning'),
      m('fieldset.flex.column.start.transfer', [
        ...
        m('fieldset.flex.row.between', [
          ...
          m('select', { onchange: (e) => { bankModel.currentAccount = e.target.value } }, [
            bankModel.accounts.forEach((account, index) => {
